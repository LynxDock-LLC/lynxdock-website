...
export const posts: Post[] = [
  {
    ...
    featured: true,
    excerpt:
      "Version 2.2's networking was built and tested but had never been watched working end to end. So we watched: three live clients, a cross-device read-state test with a human witness, a server killed mid-conversation, and a concurrency stress pass that fired sixty messages at once. The core held - and the stress test surfaced a long-message defect we fixed before shipping. This is day one of the internal alpha.",
    body: [
      ...
      "Seven of seven executed cases passed. Then we pushed harder: a concurrency stress pass where three clients fired twenty messages each into one channel simultaneously. Every accepted message got a unique id, in a single global order that all three clients agreed on to the character - no duplicates, no gaps, no lost events. Getting that right is the hard part of a distributed chat system, and it held.",
      "The stress pass also earned its keep by surfacing a real defect: very long messages were handled inconsistently, and in one path a message that exceeded the limit could vanish without telling you. We fixed it before writing this - one documented length limit enforced identically on client and server, a live character counter, a clear over-limit message, and a guarantee that the composer never clears your text unless the server has actually accepted it. We added tests at the limit, one under, and one over, and re-ran the three-client check until it was clean. That is the entire point of stress testing an alpha: find the silent failure in a controlled room, not in someone's real conversation.",
      "We are also tracking a handful of smaller, honest gaps - during a network outage the client currently blanks its cached view until reconnect (everything returns, but a local-first app should keep showing history offline), message editing exists on the server without UI yet, and a couple of cosmetic rough edges. They are on the board, in the open.",
